...
pub async fn start(state: &SharedState, id: &str) -> Result<(), Error> {
    ...
    // Persist the state before the monitor task takes ownership of the child.
    state.mark_running(id, pid).await?;
    tracing::info!(vm_id = id, pid, "VM started");
    let state = state.clone();
    let id = id.to_owned();
    tokio::spawn(monitor(child, state, id));
    Ok(())
}
...
fn signal_vm(vm: &Vm, signal: Signal, operation: &str) -> Result<(), Error> {
    let pid = vm
        .pid
        .ok_or_else(|| Error::Runtime(format!("VM has no process ID: {}", vm.id)))?;
    let pid = i32::try_from(pid).map_err(|_| Error::Runtime("invalid process ID".to_owned()))?;
    send_signal(Pid::from_raw(pid), signal)
        .map_err(|error| Error::Runtime(format!("failed to {operation} VM: {error}")))
}
...
// Consume both pipes asynchronously so Firecracker cannot block on output.
...
mod tests {
    ...
    #[test]
    fn rejects_missing_vm_artifacts() {
        let vm = Vm {
            id: "demo".to_owned(),
            name: "demo".to_owned(),
